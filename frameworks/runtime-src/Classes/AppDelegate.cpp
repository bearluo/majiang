...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    FileUtils::getInstance()->setResourceEncryptKeyAndSign("woyaopoker", "poker");
#if 0
    // use luajit bytecode package
    stack->setXXTEAKeyAndSign("woyaopoker", "poker");
    // CCLog("-------------------- bearluo ----------------,compile_scripts.sh -i src -o res/game64.zip -e xxtea_zip -ek woyaopoker -es poker -b 64--- ");
#ifdef CC_TARGET_OS_IPHONE
    if (sizeof(long) == 4) {
		// CCLog("-------------------- bearluo iphone res/game.zip ------------------- ");
        stack->loadChunksFromZIP("res/game.zip");
    } else {
		// CCLog("-------------------- bearluo iphone res/game64.zip -------------------");
        stack->loadChunksFromZIP("res/game64.zip");
    }
#else
    // android, mac, win32, etc
	// CCLog("-------------------- bearluo android, mac, win32, etc  res/game.zip ------------------- %d",stack->loadChunksFromZIP("res/game.zip"));
    stack->loadChunksFromZIP("res/game.zip");
#endif
